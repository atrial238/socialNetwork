...
import { loginUser, getCaptchaThunk, setAvatarSrc } from '../../redux/auth-reucer';
import {getUserProfile} from '../../redux/profile-reducer';
import LoginSlider from './LoginSlider/LoginSlider';
import {container, wrapper} from './Login.module.scss';
import FormLogin from './FormLogin/FormLogin';

const Login = ({ isAuth, loginUser, getCaptchaThunk, captcha, getUserProfile, setAvatarSrc }) => {
	...
	const onSubmit = async (formData) => {
		...
		if(res && res.data.resultCode === 0) {
			getUserProfile(res.data.data.id)
				.then(res => setAvatarSrc(res.data.photos.small))
		}
	};
...
export default connect(mapStateToProps, { loginUser, getCaptchaThunk, getUserProfile, setAvatarSrc})(Login);
